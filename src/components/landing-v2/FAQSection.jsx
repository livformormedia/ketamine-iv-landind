...
export default function FAQSection() {
  ...
  return (
    <section className="py-16 md:py-24 bg-gradient-to-br from-slate-900 via-slate-800 to-purple-900 relative overflow-hidden">
      {/* Decorative glow */}
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="space-y-3 md:space-y-4">
          {treatmentConfig.FAQ_ITEMS.map((faq, index) => ( // Using FAQ_ITEMS from treatmentConfig
            <div 
              key={index} 
              className="bg-gradient-to-br from-slate-900/80 to-purple-900/80 backdrop-blur-lg border-2 border-purple-500/30 rounded-2xl overflow-hidden hover:border-purple-400/50 transition-all duration-300 shadow-xl"
            >
              <button
                className="w-full px-4 md:px-6 py-4 md:py-5 flex items-center justify-between hover:bg-purple-900/30 transition-colors duration-300 text-left"
                onClick={() => setOpenIndex(openIndex === index ? -1 : index)}
              >
                <span className="font-semibold text-sm md:text-lg text-white pr-4">{faq.question}</span>
                {openIndex === index ? (
                  <ChevronUp className="w-5 h-5 md:w-6 md:h-6 text-purple-400 flex-shrink-0" />
                ) : (
                  <ChevronDown className="w-5 h-5 md:w-6 md:h-6 text-slate-400 flex-shrink-0" />
                )}
              </button>
              {openIndex === index && (
                <div className="px-4 md:px-6 py-4 md:py-5 bg-slate-900/50 border-t-2 border-purple-500/30">
